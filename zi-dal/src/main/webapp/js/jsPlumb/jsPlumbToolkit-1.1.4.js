...
(function () {
    ...
    root.jsPlumbToolkitUtil.AutoSaver = function (instance, url, headers, success, error) {
    ...
    var Base = function (data, graph) {
        ...
        Graph = exports.Graph = function (params) {
            ...
            this.serialize = function () {
                var out = { nodes: [], edges: [], ports: [], groups:[] };
                for (var i = 0; i < this.vertices.length; i++) {
                ...
                for (i = 0; i < this.groups.length; i++) {
                    out.groups.push(this.groups[i].data);
                }
        ...
        /*
         ...
         from i to j using intermediate vertices (1..k鈭�1).  Each path[i][j] is initialized to
         edgeCost(i,j).
...
(function () {
    ...
    root.jsPlumbToolkitInstance = function (params) {
        ...
        if (autoSave)
            new UTIL.AutoSaver(this, saveUrl, saveHeaders, onAutoSaveSuccess, onAutoSaveError);
         ...
         * @param {Boolean} [doNotFireEvent=false] If true, a `group:addMember` event will not be fired as a result of
         * this operation. Otherwise it will.
         */
        this.addToGroup = function(node, group, doNotFireEvent) {
...
(function () {
    ...
    exports.Circular = function (params) {
        ...
        this.step = function (toolkit, parameters) {
            var elements = [];
            Array.prototype.push.apply(elements, filterNodes(_super.adapter.getNodes()));
...
//          group event stuff
        ...
        _jbind(_e.groupMemberAdded, function(info) {
            if (!_suspendRendering && info.el.jtk.node) {
                var tkGroup = _toolkit.addToGroup(info.el.jtk.node, info.group.id, true);
                if (tkGroup) {
...
// -------------------------------- groups -----------------------------------------------------------
            ...
            var _autoSizeAGroup = function(group, groupEl, def) {
            ...
            var _autoSizeGroups = function() {
                for (var groupId in groupMap) {
                    ...
                    if (group) {
                        ...
                        if (def.autoSize) {
                            _autoSizeAGroup(group, groupMap[groupId], def);
                        }
                    }
                }
            };
...
// -------------------------------- / groups -----------------------------------------------------------
            ...
            _toolkit.bind(_e.portAdded, function (params) {
                ...
                _checkForPorts(_jsPlumb.getElement(portEl), params.node, params.node.id);
...
// ---------------------- create the default node renderer ---------------------------
                    ...
                    if (el.childNodes[i].nodeType != 3 && el.childNodes[i].nodeType != 8) {
                        ...
                        if (el.childNodes[i].tagName.toUpperCase() == els.SOURCE && el.childNodes[i].getAttribute("jtk-processed") == null) {
                            ...
                            if (portParameters.portId != null) {
                                ...
                                // to the element.
                                portMap[nodeId + "." + portParameters.portId] = el;
                                el.jtk = el.jtk || {};
                                // add a port to the node.
                            ...
                            var isSource = cn.getAttribute("is-source");
                            if (isSource === "true") {
                        ...
                        if (el.childNodes[i].tagName.toUpperCase() == els.TARGET && el.childNodes[i].getAttribute("jtk-processed") == null) {
                            ...
                            var isTarget = el.childNodes[i].getAttribute("is-target");
                            if (isTarget === "true") {
        ...
        var _getObjectInfo = function (obj) {
            ...
            return _toolkit.getObjectInfo(obj, function (obj) {
                return obj.getNode ? (portMap[obj.getFullId()] || nodeMap[obj.getNode().id]) : nodeMap[obj.id];
            });
        };
...
(function () {
    ...
    exports.Surface = function (params) {
        ...
        _super.toolkit.bind(_e.groupMemberAdded, function(info) {
            ...
            if (nodeEl) {
                // need to adjust the position of the new node. we have no real option here (in the current
                // absence of layouts inside groups) but to just place it in the middle of the group (this method
                // is only ever called after a node is added programmatically to a group).
                var uiGroup = self.getJsPlumb().getGroup(info.group.id),
                    groupSize = self.getSize(uiGroup.getDragArea()),
                    nodeSize = self.getSize(nodeEl);

                self.setPosition(info.node.id, (groupSize[0] - nodeSize[0]) / 2, (groupSize[1] - nodeSize[1]) / 2);
                ...
                drop = function (e, ui, native) {
                    ...
                    if (cont) {
                        ...
                        if (droppedOnObject != null && !isGroup && droppedOnObject.type === "Group") {
                            var uiGroup = self.getJsPlumb().getGroup(droppedOnObject.id);
...
// / NATIVE
        ...
        this.createMiniview = function (params) {
            ...
            for (var n in _super.groupMap) {
                var el = _super.groupMap[n];
...
(function () {
    ...
    exports.Miniview = function (params) {
        ...
        var _nodeMoved = function (params) {
            ...
            JP.setAbsolutePosition(params.el || _super.nodeMap[(params.node || params.group).id], params.pos);
            _zoomToFit(true);
